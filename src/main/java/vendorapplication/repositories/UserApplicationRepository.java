...
public interface UserApplicationRepository extends CrudRepository<UserApplicationEntity,Integer> {
     ...
     * @param userId
     * @return
     * SELECT app_id,user_id, app_action_dc, app_dc_date, app_action_dfo, app_dfo_date, app_action_bdo, app_bdo_date,user_application.createddate,
     *        ventype_name, vendortype_name
     * from public.user_application
     * INNER JOIN mst_vendor ON user_application.ventype_id = mst_vendor.ventype_id
     * INNER JOIN mst_vendortype ON user_application.vendortype_id = mst_vendortype.vendortype_id
     * where user_id =15 AND user_application.active = true  order by createddate desc
     */
    ...
    @Query(value = "SELECT app_id, app_action_dc, app_dc_date, app_action_dfo, app_dfo_date, app_action_bdo, app_bdo_date,user_application.createddate,ventype_name, vendortype_name  from public.user_application INNER JOIN mst_vendor ON user_application.ventype_id = mst_vendor.ventype_id INNER JOIN mst_vendortype ON user_application.vendortype_id = mst_vendortype.vendortype_id where user_id =:userId AND user_application.active = true  order by createddate desc", nativeQuery = true)
        ...
        // @Query(value = "SELECT app_id, app_action_dc, app_action_dfo, app_action_bdo,createddate  from public.user_application where user_id =:userId AND active = true  order by createddate desc", nativeQuery = true)
